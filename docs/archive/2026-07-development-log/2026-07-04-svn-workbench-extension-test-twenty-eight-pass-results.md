...
| 用例 | 覆盖点 |
| --- | --- |
| lists editable AI provider presets | 校验 DeepSeek、通义、智谱 Coding、智谱通用、Kimi、自定义预设均存在，并且 Base URL 不包含 `/chat/completions`。 |
| validates AI provider configuration | 校验 Base URL 去尾斜杠，以及 Base URL / model / API Key 缺失时必须阻止连接。 |
...
AI 模型配置与 SecretStorage 入口已经进入自动化测试基线。

配置页的“测试连接”需要用户填写真实 API Key 后在本机手动验证；自动化测试不会访问真实模型服务。
