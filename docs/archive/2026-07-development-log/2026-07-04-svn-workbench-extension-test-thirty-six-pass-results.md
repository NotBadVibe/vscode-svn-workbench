...
PASS parses lightweight svn diff summary
PASS attaches diff summary to commit message AI request
PASS builds commit message AI request in template completion mode
PASS preserves user commit message template fields
...
| 用例 | 覆盖点 |
| --- | --- |
| builds commit message AI request in template completion mode | AI 模板补全请求携带模板、当前内容和已选文件。 |
| preserves user commit message template fields | AI 或本地规则补全不能覆盖用户已填写字段。 |
...
AI 提交说明模板补全已经进入自动化测试基线。

本轮重点是保护用户已写内容，AI 只补空字段。
