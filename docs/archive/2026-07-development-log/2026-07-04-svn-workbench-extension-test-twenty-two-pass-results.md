# SVN Workbench Extension Host 二十二项测试通过记录
...
| 测试 | 验证点 |
| --- | --- |
| parses svn conflict info xml | 能解析 `svn info --xml` 中的 Base/Mine/Theirs。 |
| collects conflict items from validation working copy | 能从真实 SVN 冲突工作副本收集冲突文件。 |
...
该提示来自 VS Code 测试宿主单实例互斥，最终退出码为 0，测试通过。
