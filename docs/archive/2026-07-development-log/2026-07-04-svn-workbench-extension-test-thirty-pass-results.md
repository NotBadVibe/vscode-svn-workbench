...
| 用例 | 覆盖点 |
| --- | --- |
| resolves AI scenario model overrides | 校验场景模型留空时继承默认模型，有覆盖值时使用场景模型。 |
| parses OpenAI-compatible model list | 校验 `/models` 标准返回结构可解析、过滤无效项并排序。 |
...
AI 场景模型路由和模型列表解析已经进入自动化测试基线。

模型列表真实拉取依赖用户 API Key 和供应商是否支持 `/models`，自动化测试只覆盖解析逻辑，不访问真实模型服务。
