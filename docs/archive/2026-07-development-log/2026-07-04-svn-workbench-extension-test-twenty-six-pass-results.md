...
| 用例 | 覆盖点 |
| --- | --- |
| builds bounded conflict AI request | AI 冲突请求只读取有限内容片段，避免大文件直接进入模型上下文。 |
| keeps conflict AI advice decision only | Working 仍有冲突标记时，AI 本地规则必须建议人工合并，不允许自动放行。 |
...
AI 冲突建议最小闭环已经进入自动化测试基线。

本轮仍未执行真实 `svn resolve` 写操作，冲突验证工作副本保持可重复测试状态。
