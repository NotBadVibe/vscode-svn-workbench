...
> 验收日期：2026-07-31  
> 结论：本机开发验收通过；推送发布前仍需取得三平台 CI 与目标企业环境签字  
> 功能追踪：[`../SVN工作台原型v3/实现状态与验收追踪.md`](../SVN工作台原型v3/实现状态与验收追踪.md)  
> 页面截图：[`artifacts/2026-07-31/pages/README.md`](./artifacts/2026-07-31/pages/README.md)
> 中文体验专项记录：[`2026-07-31-中文体验与页面滚动验收.md`](./2026-07-31-中文体验与页面滚动验收.md)

## 1. 候选与环境

| 项目 | 值 |
| --- | --- |
| 候选源码 | 当前工作区快照；仓库尚无可引用提交与 remote，不伪造 commit ID |
...
| 一键门禁 | `SVN_WORKBENCH_RELEASE_DATE=2026-07-31 npm run verify` | 通过 | 上述层级按顺序全部通过 |
| 依赖安全 | `npm audit --audit-level=high` | 通过 | 0 vulnerabilities |
| 打包 | `npm run package:vsix` | 通过 | 91 文件，vsce 显示 431.21 KB |
...
逐项实现与边界见 [`实现状态与验收追踪.md`](../SVN工作台原型v3/实现状态与验收追踪.md)。

## 4. 性能

| 指标 | 实测 | 预算 | 结论 |
| --- | ---: | ---: | --- |
| Mock 首次可交互 P50 | 45ms | — | 记录 |
| Mock 首次可交互 P95 | 49ms | ≤700ms | 通过 |
| Shell JS gzip | 34,056B | ≤163,840B | 通过 |
| Shell CSS gzip | 17,884B | ≤51,200B | 通过 |
| 业务懒加载块 | 19 | ≥17 | 通过 |
| Repository 子任务异步块 | 7 | ≥7 | 通过 |
| 5000 文件已挂载行 | 18 | ≤100 | 通过 |
| 5000 文件滚动到末尾 | 38ms | ≤500ms | 通过 |

原始 20 次样本与预算判断见 [`performance.json`](./artifacts/2026-07-31/performance.json)。脚本在超预算时返回非零退出码，不依赖人工判断。
...
截图目录：[`artifacts/2026-07-31/`](./artifacts/2026-07-31/)；逐页说明：[`pages/README.md`](./artifacts/2026-07-31/pages/README.md)。
...
安装生命周期的可归档摘要见 [`vsix-install.json`](./artifacts/2026-07-31/vsix-install.json)。

真实 VS Code 的右键菜单、任务直达与局部滚动现场检查单独记录在 [`中文体验与页面滚动验收记录`](./2026-07-31-中文体验与页面滚动验收.md)；自动化或 Mock 结果不替代其中的人工项。

## 8. 平台边界与发布前外部证据

- 当前工作区没有 Git remote，无法在本次本机执行中提供 Windows/Linux GitHub Actions 运行链接；`.github/workflows/verify.yml` 已将 Windows、macOS、Linux 的 SVN、覆盖率、Webview 与 Extension Host 验收设为强制候选门禁。
...
- 当前源码尚无可引用 commit，因此本报告是工作区候选证据，不是已推送不可变发布记录。
...
- [ ] Windows/macOS/Linux 远端 CI 运行全部绿色
- [ ] 目标企业 AI/代理/CA（若项目要求）连接性签字
- [ ] 冻结并推送候选 commit

当前候选可交付给产品/测试进行独立安装验收。对外发布前，发布负责人必须补齐后三项适用证据；若任一项失败，结论自动退回“不通过”，修复后从受影响层重新执行。
