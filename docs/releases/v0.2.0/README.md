...
- [x] 两个外部入口都到相同准确预览，四角色与路径一致。
- [x] 未保存草稿不会被静默当成工具输入或覆盖。
- [x] Windows/macOS/Linux 配置路径与参数校验沿用现有安全单测。
- [x] 缺工具与工具退出后下一步清楚。
...
- [x] 无冲突更新后查看本地修改打开 Changes。
- [x] 有冲突更新仍优先显示准确数量与直达。
- [x] 来源编辑器关闭后按钮不承诺无法完成的返回。
- [x] 结果动作不额外发出写命令。
