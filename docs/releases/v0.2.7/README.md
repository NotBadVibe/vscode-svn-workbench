...
- **R53 终审修复中**：前序基线（`scripts/measure-v027-r53.js`，`.validation/evidence/v0.2.x/`，工作区 gitignored）发现三项 P0 与一项 P1 缺口，收口中——① tier-500 降级出口 P95 约 3400~~4600ms（超 500ms 预算 6~~9 倍，2×rAF 推迟的同步挂载仍卡住出口，改按需加载）；②脚本曾硬编码 `passed:true`（改逐档断言，超预算 exit 非零）；③冲突草稿 ACK 未绑定目标（已绑定 relativePath+单调 revision，乱序拒绝）；④连续切换 100 次/200 字符连续输入/焦点保持已补入脚本探针。完整视图未达标档位（5000/10000 行、100 块动作）如实记失败，不放宽断言。
...
| 任务              | 自动化证据                                                                      |
| ----------------- | ------------------------------------------------------------------------------- |
| 30 混合修改选提交 | `daily-path.spec.ts` + `commit-handoff.spec.ts`（Changes→Diff→Commit 全链）     |
| 20 文件审阅       | `review-queue.spec.ts`（队列建立/导航/标已看/返回恢复）                         |
| 多冲突处理        | `conflict-v013.spec.ts` + `v027-save-keyboard.spec.ts`（编辑→保存→预览→确认前） |
| 旧历史查询        | `v021` 系列（按条件查询/超 300 条窗口化）                                       |
| 重启恢复搁置      | `PatchShelfTask.test.ts` + `shelfIndex.test.ts`（索引/迁移/恢复链）             |
