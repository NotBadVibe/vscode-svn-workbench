/*
 * Shiki 按需子集 shim（体积验证用）。
 *
 * @pierre/diffs 通过 `import { bundledLanguages } from "shiki"` 解析语言
 * （dist/highlighter/languages/resolveLanguage.js:2），shiki 根模块的
 * bundledLanguages 是全量注册表，Vite 会把全部 ~200 个语法打成 chunk
 * （按需加载但全部进产物）。本 shim 通过 Vite resolve.alias 精确替换
 * "shiki" 模块，只注册规划 §10 建议的语言子集；其余符号原样转发
 * shiki/core 与两个 engine 入口，行为不变。
 *
 * 注意：alias 为正则精确匹配 /^shiki$/，"shiki/core"、"shiki/textmate"、
 * "shiki/engine/*" 等子路径不受影响，仍指向真实模块。
 */
