...
const snapshot: CommitSnapshot = {
  ...
  files: [
    ...
    { relativePath: 'dist/out.js', status: 'unversioned', selection: 'excluded' }
  ],
  ...
  aiPrivacy: [
    { scenario: 'selection', model: 'local', fileLimit: 200, data: 'metadata', historyIncluded: false },
    { scenario: 'message', model: 'local', fileLimit: 80, data: 'statistics', historyIncluded: false }
  ],
...
describe('CommitModule', () => {
  it('keeps excluded files disabled and executes only with the preview token', async () => {
    ...
    expect(onAction).toHaveBeenCalledWith('commit/execute', { previewToken: 'preview-1' });
  });
  ...
  it('中文输入法选词期间 Ctrl+Enter 不生成提交预览', async () => {
    ...
    expect(onAction).not.toHaveBeenCalledWith('commit/preview', expect.anything());

    await fireEvent.keyDown(input, { key: 'Enter', ctrlKey: true });
    expect(onAction).toHaveBeenCalledWith('commit/preview', { selectedPaths: ['src/a.ts'], message: 'feat(core): update' });
  });
});
