...
const snapshot = {
  ...
  files: [
    { relativePath: 'src/a.ts', status: 'modified' as const, selection: 'selected' as const, reason: '本地修改' },
    { relativePath: 'src/b.ts', status: 'conflicted' as const, selection: 'blocked' as const, reason: '存在冲突' }
  ],
...
describe('ChangesModule', () => {
  it('filters files and opens a diff from the visible row', async () => {
    ...
    await fireEvent.input(screen.getByLabelText('筛选变更文件'), { target: { value: 'a.ts' } });
    expect(screen.getByText('src/a.ts')).toBeInTheDocument();
    expect(screen.queryByText('src/b.ts')).not.toBeInTheDocument();

    await fireEvent.click(screen.getByRole('button', { name: '查看 src/a.ts 差异' }));
    expect(onAction).toHaveBeenCalledWith('open-diff', { relativePath: 'src/a.ts' });
  });
  ...
  it('中文、空格、括号和井号路径可以筛选并原样打开', async () => {
    ...
    render(ChangesModule, {
      snapshot: {
        ...snapshot,
        files: [{ relativePath, status: 'modified' as const, selection: 'selected' as const, reason: '本地修改' }],
        summary: { modified: 1 }
      },
      onAction
    });
    await fireEvent.input(screen.getByLabelText('筛选变更文件'), { target: { value: '终版' } });
    await fireEvent.click(screen.getByRole('button', { name: `查看 ${relativePath} 差异` }));
  ...
  it('virtualizes a 5000-file working copy instead of mounting every row', () => {
    ...
    expect(screen.getByRole('list', { name: 'SVN 变更文件' })).toHaveClass('file-list--virtual');
  ...
  it('在 Changes 中编辑并保存与提交模块共享的 Host 草稿', async () => {
    ...
    await fireEvent.input(screen.getByLabelText('共享提交草稿'), { target: { value: 'fix: shared draft' } });
    await fireEvent.click(screen.getByRole('button', { name: '保存共享草稿' }));
    expect(onAction).toHaveBeenCalledWith('commit/update-draft', { message: 'fix: shared draft' });
  });
  ...
  it('Revert 必须展示不可恢复说明并完成二次勾选', async () => {
    ...
    render(ChangesModule, {
      snapshot: {
        ...snapshot,
        operationPreview: {
          token: 'revert-1', operation: 'revert' as const, paths: ['src/a.ts'], command: 'svn revert "src/a.ts"',
          consequences: ['丢弃本地变更。'], destructive: true, recoverability: 'SVN 无法恢复。', canExecute: true, issues: []
        }
      },
      onAction
    });
    ...
    await fireEvent.click(screen.getByRole('checkbox', { name: /逐项核对文件清单/ }));
    await fireEvent.click(execute);
    expect(onAction).toHaveBeenCalledWith('changes/execute-operation', { previewToken: 'revert-1' });
  });
});
