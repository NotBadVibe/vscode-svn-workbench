...
import { classifySvnFailure, extractSvnCertificateDetails } from '../../src/svn/svnErrorClassifier';
...
`);
  ...
  it.each([
    ...
    expect(classifySvnFailure(message)).toEqual(expect.objectContaining({ category: 'network', networkKind: kind }));
  });
  ...
  it('routes locked and interrupted working copies to the recovery module', () => {
    expect(classifySvnFailure('working copy is locked')).toEqual(expect.objectContaining({ recoveryModule: 'repository' }));
    expect(classifySvnFailure('previous operation has not finished')).toEqual(expect.objectContaining({ recoveryModule: 'repository' }));
  });
});
