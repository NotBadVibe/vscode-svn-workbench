...
describe("SVN security context", () => {
  ...
  it("builds explicit one-time certificate trust without caching auth data", () => {
    ...
    expect(invocation.args).toContain(
      "--trust-server-cert-failures=unknown-ca,expired",
    );
  ...
  it("resolves the most specific registered repository context", () => {
    setSvnSecurityContext(root, {
      authentication: { username: "root", password: "one" },
    });
    setSvnSecurityContext(path.join(root, "external"), {
      authentication: { username: "external", password: "two" },
    });

    expect(
      resolveSvnSecurityContext(path.join(root, "src"))?.authentication
        ?.username,
    ).toBe("root");
    expect(
      resolveSvnSecurityContext(path.join(root, "external", "src"))
  ...
  it("覆盖空上下文、已有非交互参数、永久纯证书和非法失败类型", () => {
    expect(buildSvnSecurityInvocation(["status"], undefined)).toEqual({
      args: ["status"],
      safeArgs: ["status"],
    });
    ...
    expect(
      auth.args.filter((item) => item === "--non-interactive"),
    ).toHaveLength(1);
    ...
    expect(certificateOnly.args).not.toContain(
      "servers:global:store-passwords=no",
    );
    expect(certificateOnly.args).toContain(
      "--trust-server-cert-failures=other",
    );
    expect(
      normalizeCertificateFailures(["expired", "expired", "invalid" as never]),
    ).toEqual(["expired"]);
  });
