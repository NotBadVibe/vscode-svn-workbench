...
describe("runSvnCommand", () => {
  ...
  it("writes credentials to stdin while keeping observable command arguments redacted", async () => {
    const cwd = await fs.mkdtemp(
      path.join(os.tmpdir(), "svn-runner-security-"),
    );
    const script = path.join(cwd, "read-stdin.js");
    await fs.writeFile(
      script,
      `let value='';process.stdin.setEncoding('utf8');process.stdin.on('data',c=>value+=c);process.stdin.on('end',()=>console.log(JSON.stringify({length:value.trim().length,args:process.argv.slice(2)})));`,
      "utf8",
    );
    setSvnSecurityContext(cwd, {
      authentication: {
    ...
    try {
      ...
      expect(JSON.parse(result.stdout)).toEqual(
        expect.objectContaining({ length: 12 }),
      );
    } finally {
