...
describe("diffAtomicWriter Windows 覆盖回退（v0.0.6 平台修复）", () => {
  let filePath: string;

  beforeEach(async () => {
    workRoot = await fs.mkdtemp(path.join(os.tmpdir(), "svn-diffwin-"));
    filePath = path.join(workRoot, "target.txt");
  });
  ...
  /** 构造可注入 fileOps；renameImpl 可模拟 Windows 失败语义。 */
  function fakeOps(renameImpl: (from: string, to: string) => Promise<void>): {
    ops: DiffAtomicFileOps;
    calls: Array<{ op: string; from: string; to: string }>;
  } {
    const calls: Array<{ op: string; from: string; to: string }> = [];
    return {
      calls,
      ops: {
        writeAndSync: async (tempPath, data, mode) => {
          await fs.writeFile(
            tempPath,
            data,
        ...
        rm: async (tempPath) => {
          calls.push({ op: "rm", from: tempPath, to: "" });
  ...
  function eperm(): NodeJS.ErrnoException {
    const err = new Error(
      "模拟 Windows 覆盖冲突 EPERM",
    ) as NodeJS.ErrnoException;
    err.code = "EPERM";
    return err;
  }

  it("Windows 覆盖冲突时经唯一 backup 回退替换成功并清理 backup（旧实现红）", async () => {
    const original = Buffer.from("original\n", "utf8");
    await fs.writeFile(filePath, original);
    let tempTargetAttempts = 0;
    const { ops, calls } = fakeOps(async (from, to) => {
      if (from.endsWith(".tmp") && to === filePath) {
        tempTargetAttempts += 1;
        if (tempTargetAttempts === 1) throw eperm();
      }
      await fs.rename(from, to);
    });
    const writer = new DiffAtomicWriter(ops, "win32");
    const result = await writer.writeRawBytes({
      targetPath: filePath,
    ...
    // 回退路径：temp→target 首试失败后 target→backup、temp→target、backup 清理。
    expect(tempTargetAttempts).toBeGreaterThan(1);
    const hasBackupRename = calls.some(
      (c) => c.op === "rename" && c.to.endsWith(".bak"),
    );
    expect(hasBackupRename).toBe(true);
    const entries = await fs.readdir(workRoot);
    expect(
      entries.filter((n) => n.endsWith(".tmp") || n.endsWith(".bak")),
    ).toEqual([]);
  });

  it("target→backup 失败时原文件不变且无残留", async () => {
    const original = Buffer.from("original\n", "utf8");
    await fs.writeFile(filePath, original);
    const { ops, calls } = fakeOps(async (from, to) => {
      if (from.endsWith(".tmp") && to === filePath) throw eperm();
      if (to.endsWith(".bak")) {
        const err = new Error("模拟 EACCES") as NodeJS.ErrnoException;
        err.code = "EACCES";
        throw err;
      }
      await fs.rename(from, to);
    ...
    expect(calls.some((c) => c.op === "rm")).toBe(true);
  });

  it("temp→target 回退重试失败时精确恢复原文件并清理 backup", async () => {
    const original = Buffer.from("original\n", "utf8");
    await fs.writeFile(filePath, original);
    const { ops, calls } = fakeOps(async (from, to) => {
      // temp→target 始终失败（首试与回退重试都失败）
      if (from.endsWith(".tmp") && to === filePath) throw eperm();
      await fs.rename(from, to);
    });
    const writer = new DiffAtomicWriter(ops, "win32");
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) expect(result.reason).toBe("writeFailed");
    // 精确恢复：target 内容与备份前完全一致。
    expect(await fs.readFile(filePath, "utf8")).toBe("original\n");
    const entries = await fs.readdir(workRoot);
    expect(
      entries.filter((n) => n.endsWith(".tmp") || n.endsWith(".bak")),
    ).toEqual([]);
    // 恢复由 backup→target 完成。
    expect(
      calls.some((c) => c.op === "rename" && c.from.endsWith(".bak")),
    ).toBe(true);
  });

  it("非 Windows 平台不启用 backup 回退（直接失败且不产生 backup 尝试）", async () => {
    const original = Buffer.from("original\n", "utf8");
    await fs.writeFile(filePath, original);
    const { ops, calls } = fakeOps(async (from, to) => {
      if (from.endsWith(".tmp") && to === filePath) throw eperm();
    ...
    const writer = new DiffAtomicWriter(ops, "linux");
    const result = await writer.writeRawBytes({
      targetPath: filePath,
      bytes: Buffer.from("new\n", "utf8"),
      expectedRawHash: hashBytes(original),
    ...
    expect(hasBackupRename).toBe(false);
    expect(await fs.readFile(filePath, "utf8")).toBe("original\n");
  });

  it("临时文件落盘失败（模拟 Windows 只读句柄 fsync EACCES）时 writeFailed 且保留原文件", async () => {
    const original = Buffer.from("original\n", "utf8");
    await fs.writeFile(filePath, original);
    const ops: DiffAtomicFileOps = {
      writeAndSync: async () => {
        const err = new Error("模拟 EACCES") as NodeJS.ErrnoException;
        err.code = "EACCES";
        throw err;
