...
describe('SVN credential SecretStorage', () => {
  it('stores credentials under a hashed repository key and can remove them', async () => {
    ...
    await storeSvnCredential(secrets, identity, { username: 'alice', password: 'secret' });
    expect(await readStoredSvnCredential(secrets, identity)).toEqual({ username: 'alice', password: 'secret' });
