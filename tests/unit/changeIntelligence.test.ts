...
} from "../../src/ai/changeIntelligence";
...
describe("change intelligence", () => {
  ...
  it("根据 UI 变更生成浏览器验收建议", () => {
    const impact = buildLocalImpactAnalysis([
      candidate("src/webview/App.svelte"),
    ]);
    expect(impact.tests.map((item) => item.command)).toContain(
  ...
  it("覆盖生成物、调试代码、截断、不可读和跳过状态", async () => {
    vi.mocked(fs.readFile).mockImplementation(async (filePath) => {
      ...
      if (value.endsWith("secret.ts"))
        return `${"x".repeat(160_010)}\n-----BEGIN PRIVATE KEY-----` as never;
      if (value.endsWith("debug.ts"))
        return 'line1\nconsole.debug("visible line");\n' as never;
      if (value.endsWith("unreadable.ts")) throw new Error("denied");
      return "ok" as never;
    });
    const excluded = {
      ...candidate("dist/generated.ts"),
      generatedDecision: "exclude" as const,
    };
    const deleted = {
      ...candidate("src/deleted.ts"),
      status: "deleted" as const,
    };
    const missing = {
      ...candidate("src/missing.ts"),
      status: "missing" as const,
    };
    const blocked = {
      ...candidate("src/blocked.ts"),
    ...
    const review = await buildLocalChangeReview([
      ...
      blocked,
      binary,
      candidate("src/secret.ts"),
      candidate("src/debug.ts"),
      candidate("src/unreadable.ts"),
      { ...candidate("tests/unit/a.test.ts"), templateGroup: "other" },
    ]);
    expect(review.findings.some((item) => item.category === "generated")).toBe(
      true,
    );
    expect(
      review.findings.some(
        (item) => item.category === "debug" && item.line === 2,
      ),
    ).toBe(true);
  ...
  it("表达空范围、各影响区风险、删除观察点和三类测试建议", async () => {
    ...
    const many = Array.from({ length: 8 }, (_, index) =>
      candidate(`src/extension/f${index}.ts`),
    );
    const impact = buildLocalImpactAnalysis([
      ...many,
    ...
    expect(impact.areas.find((item) => item.id === "src/extension")?.risk).toBe(
      "high",
    );
    expect(impact.areas.find((item) => item.id === "src/webview")?.title).toBe(
      "Svelte Webview",
    );
    expect(impact.areas.find((item) => item.id === "tests")?.title).toBe(
      "自动化测试",
    );
    expect(impact.areas.find((item) => item.id === "docs")?.title).toBe(
      "产品与开发文档",
    );
    expect(impact.tests.map((item) => item.command)).toEqual(
    ...
    expect(
      impact.observations.some((item) => item.includes("包含删除项")),
    ).toBe(true);
