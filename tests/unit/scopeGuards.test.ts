...
import { createScopeFromExplorer, type OperationScope } from '../../src/scope/operationScope';
import { isPathInScope, validatePathsInScope } from '../../src/scope/pathBoundaryGuard';
...
describe('右键操作范围边界', () => {
  it('区分单文件、目录本身、子项、同前缀兄弟和父目录', () => {
    const fileScope: OperationScope = {
      id: 'f', repositoryRoot: '/repo', source: 'explorerFile',
      roots: [{ absolutePath: '/repo/src/a.ts', relativePath: 'src/a.ts', kind: 'file' }],
    ...
    const folderScope = { ...fileScope, roots: [{ absolutePath: '/repo/src/a', relativePath: 'src/a', kind: 'folder' as const }] };
    ...
    expect(validatePathsInScope(folderScope, ['/repo/src/a/x', '/outside'])).toEqual({
      validItems: [path.resolve('/repo/src/a/x')], outOfScopeItems: [path.resolve('/outside')]
    });
  });
  ...
  it('从文件和混合多选创建范围，并只合并真正被父目录覆盖的子项', async () => {
    ...
    const fileScope = await createScopeFromExplorer(root, Uri.file(path.join(root, 'file.ts')) as never);
    expect(fileScope.source).toBe('explorerFile');
    const mixed = await createScopeFromExplorer(root, Uri.file(path.join(root, 'file.ts')) as never, [
      Uri.file(path.join(root, 'file.ts')), Uri.file(path.join(root, 'folder')), Uri.file(path.join(root, 'folder', 'child.ts'))
    ] as never);
    expect(mixed.source).toBe('explorerMultiSelection');
    expect(mixed.roots.map((item) => item.relativePath).sort()).toEqual(['file.ts', 'folder']);
  });
});
