...
async function assertScrollable(region: Locator, lastItem?: Locator): Promise<void> {
  await expect(region).toBeVisible();
  await region.evaluate((element) => { element.scrollTop = 0; element.scrollLeft = 0; element.dispatchEvent(new Event('scroll')); });
  ...
  expect(metrics.scrollHeight, `滚动区 ${await region.getAttribute('aria-label')} 没有形成真实溢出`).toBeGreaterThan(metrics.clientHeight + 1);
  expect(['auto', 'scroll']).toContain(metrics.overflowY);
  expect(metrics.scrollbarColor).not.toBe('auto');

  await region.focus();
  await region.press('PageDown');
  await expect.poll(() => region.evaluate((element) => element.scrollTop)).toBeGreaterThan(metrics.scrollTop);
  await region.evaluate((element) => { element.scrollTop = element.scrollHeight; element.dispatchEvent(new Event('scroll')); });

  if (lastItem) {
    const [containerBox, itemBox] = await Promise.all([region.boundingBox(), lastItem.boundingBox()]);
    expect(containerBox).not.toBeNull();
    expect(itemBox).not.toBeNull();
    expect(itemBox!.y + itemBox!.height).toBeLessThanOrEqual(containerBox!.y + containerBox!.height + 1);
...
test('SCR-01/02/09/10/11：小视口下 Shell、导航和变更列表完整可达', async ({ page }) => {
  ...
  const railMetrics = await rail.evaluate((element) => ({ clientHeight: element.clientHeight, scrollHeight: element.scrollHeight }));
  if (railMetrics.scrollHeight > railMetrics.clientHeight) await rail.evaluate((element) => { element.scrollTop = element.scrollHeight; });
...
test('SCR-03：提交双栏分别滚动，底部确认操作可达', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: '提交说明' }).fill('feat(中文界面): 验证小区域滚动');
  await page.getByRole('button', { name: '生成提交预览' }).click();
  const compose = page.getByRole('region', { name: '提交说明与提交前检查' });
  await assertScrollable(compose, page.getByRole('button', { name: '确认提交' }));
...
test('SCR-05：冲突列表、编辑器正文和解决确认在小高度下可达', async ({ page }) => {
  await page.setViewportSize({ width: 1024, height: 600 });
  await page.goto('/?dataset=scroll');
  ...
  expect(await editorScroller.evaluate((element) => element.scrollHeight)).toBeGreaterThan(await editorScroller.evaluate((element) => element.clientHeight));
  ...
  await assertScrollable(workspace, page.getByRole('button', { name: '确认使用当前工作副本内容并标记解决' }));
});
...
test('SCR-06：变更集三栏拥有独立滚动归属且选择不丢失', async ({ page }) => {
  ...
  await assertScrollable(suggestions, suggestions.locator('.split-card').last());
  ...
  await assertScrollable(selectedPaths, selectedPaths.locator(':scope > div').last());
  await expect(editor.getByText(/将分组的文件（36）/)).toBeVisible();
});

test('SCR-07：仓库子任务按需显示，目录与属性列表均可滚到末项', async ({ page }) => {
  await page.setViewportSize({ width: 1024, height: 600 });
  await page.goto('/?dataset=scroll');
  ...
  await expect.poll(() => page.evaluate(() => performance.getEntriesByType('resource').some((entry) => /BrowseTask-.*\.js/.test(entry.name)))).toBe(true);
  ...
  await expect.poll(() => page.evaluate(() => performance.getEntriesByType('resource').some((entry) => /PropertiesTask-.*\.js/.test(entry.name)))).toBe(true);
});
...
test('SCR-08：设置和诊断长列表具有明确滚动区', async ({ page }) => {
  ...
  await assertScrollable(acceptance, acceptance.locator('.acceptance-section').last());
});

test('ZH-01/02/03/04/09：公共状态、数量、时间和 AI 外发说明符合中文习惯', async ({ page }) => {
  await page.goto('/');
  ...
  for (const moduleName of ['提交', '历史', '冲突', 'AI 审查', '影响分析', '仓库操作', '设置', '诊断']) {
    await openModule(page, moduleName);
    const pureEnglishControls = await page.locator('h1, h2, button, .eyebrow').evaluateAll((elements) => elements
      .map((element) => (element.textContent ?? '').trim())
      .filter((text) => /^[A-Za-z][A-Za-z\s-]{2,}$/.test(text))
      .filter((text) => !['AI', 'SVN', 'BASE', 'DeepSeek', 'vscode-svn'].includes(text)));
    expect(pureEnglishControls, `${moduleName} 存在未纳入白名单的英文业务文案`).toEqual([]);
  }
  await openModule(page, '提交');
...
test('SCR-12/13/14/15 与 ZH-10：高度和 100%～200% 缩放矩阵无永久裁切', async ({ page }) => {
  const matrix = [
  ...
  for (const item of matrix) {
    await test.step(item.zoom, async () => {
      ...
      await action.focus();
      const [contentBox, actionBox] = await Promise.all([page.locator('.workbench-content').boundingBox(), action.boundingBox()]);
