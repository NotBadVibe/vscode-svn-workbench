...
test('opens the mock changes workspace and navigates to diff', async ({ page }) => {
  await page.goto('/');
  ...
  await expect(page.getByRole('heading', { name: '查看本地修改' })).toBeVisible();
  await expect(page.getByText('BASE ↔ 工作副本 · typescript')).toBeVisible();
});

test('has no automatically detectable accessibility violations on changes', async ({ page }) => {
  await page.goto('/');
  await expect(page.getByRole('heading', { name: '工作副本修改' })).toBeVisible();
...
test('previews a commit before enabling execution', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: '提交当前范围' })).toBeVisible();
  ...
  await expect(page.getByRole('textbox', { name: '提交说明' })).toHaveValue('feat(workbench): 迁移统一 Svelte UI');
...
test('keeps AI file selection advisory and user-editable', async ({ page }) => {
  ...
  await expect(page.getByText('建议选择 1 个文件；1 个需要人工确认，1 个建议排除。')).toBeVisible();
  await expect(page.getByLabel('选择 src/extension.ts')).toBeChecked();
  await expect(page.getByLabel('选择 src/webview/App.svelte')).not.toBeChecked();
  await page.getByLabel('选择 src/webview/App.svelte').check();
  await expect(page.getByLabel('选择 src/webview/App.svelte')).toBeChecked();
});

test('compares two revisions inside the unified workbench', async ({ page }) => {
  await page.goto('/');
...
test('keeps conflict advice separate from explicit resolve', async ({ page }) => {
  await page.goto('/');
  ...
  await expect(page.getByRole('button', { name: '确认使用当前工作副本内容并标记解决' })).toBeEnabled();
});

test('opens Svelte settings without exposing the stored API key', async ({ page }) => {
  await page.goto('/');
...
test('shows environment diagnostics and expandable acceptance evidence', async ({ page }) => {
  await page.goto('/');
  await page.getByRole('button', { name: '诊断', exact: true }).click();
  await expect(page.getByRole('heading', { name: '环境诊断', exact: true }).last()).toBeVisible();
...
test('requires an update preview before running svn update', async ({ page }) => {
  await page.goto('/');
  await page.getByRole('button', { name: '仓库操作', exact: true }).click();
  await expect(page.getByRole('heading', { name: '更新当前范围' }).first()).toBeVisible();
...
test('shows review evidence without rendering sensitive values', async ({ page }) => {
  await page.goto('/');
  await page.getByRole('button', { name: 'AI 审查', exact: true }).click();
  await expect(page.locator('.intelligence-page').getByRole('heading', { name: 'AI 变更审查' })).toBeVisible();
...
test('turns a split suggestion into a previewed SVN changelist', async ({ page }) => {
  await page.goto('/');
...
test('runs the agent plan only through explicit step approvals', async ({ page }) => {
  await page.goto('/');
  ...
  for (let index = 0; index < 3; index += 1) await page.getByRole('button', { name: '批准此步' }).first().click();
  await expect(page.getByText('受控分析计划已完成，可以进入审查、影响或提交模块继续操作。')).toBeVisible();
});

test('uses an accessible Svelte context menu and explicit file-operation preview', async ({ page }) => {
  await page.goto('/');
  const row = page.locator('.file-row').filter({ hasText: 'dist/debug.log' });
  await row.click({ button: 'right' });
  await expect(page.getByRole('menu', { name: 'dist/debug.log 操作菜单' })).toBeVisible();
  await page.getByText('加入版本控制', { exact: true }).click();
  await expect(page.getByRole('dialog', { name: 'SVN 文件操作预览' })).toBeVisible();
  await page.getByRole('button', { name: '确认加入版本控制' }).click();
  await expect(page.getByText('1 个文件已加入版本控制。请刷新并确认最新 SVN 状态。')).toBeVisible();
});

test('supports keyboard navigation and dismissal in the Svelte context menu', async ({ page }) => {
  await page.goto('/');
...
test('requires explicit confirmation when restoring a file revision', async ({ page }) => {
  await page.goto('/');
  ...
  await expect(page.getByRole('dialog', { name: '修订恢复预览' })).toContainText('不会自动提交');
  await page.getByRole('button', { name: '确认覆盖工作副本文件' }).click();
  await expect(page.getByText('src/extension.ts 已恢复为 r42 内容；尚未提交。')).toBeVisible();
});
...
test('previews SVN property changes before applying them', async ({ page }) => {
  ...
  await expect(page.getByText('svn propset "svn:ignore" <value> "."')).toBeVisible();
  await page.getByRole('button', { name: '确认设置属性' }).click();
  await expect(page.getByText('已设置属性 svn:ignore；变更尚未提交。')).toBeVisible();
});

test('makes working-copy cleanup an explicit non-destructive confirmation', async ({ page }) => {
  await page.goto('/');
  await page.getByRole('button', { name: '仓库操作' }).click();
  ...
  await expect(page.getByText('清理已完成；未删除未版本化文件，请重新检查状态。')).toBeVisible();
});
