...
import { parseSvnChangelistsXml, type SvnChangelistGroup } from './svnChangelistParser';

export { parseSvnChangelistsXml } from './svnChangelistParser';
export type { SvnChangelistGroup } from './svnChangelistParser';

export async function collectSvnChangelists(svnPath: string, scope: OperationScope): Promise<SvnChangelistGroup[]> {
  const result = await runSvnCommand(svnPath, ['status', '--xml', ...scope.roots.map((root) => root.absolutePath)], scope.repositoryRoot);
...
export async function applySvnChangelist(
  ...
  const args = name ? ['changelist', name, ...absolutePaths] : ['changelist', '--remove', ...absolutePaths];
