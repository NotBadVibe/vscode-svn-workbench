...
import { buildSvnSecurityInvocation, resolveSvnSecurityContext } from '../security/svnSecurityContext';
...
export function runSvnCommand(
  ...
  const invocation = buildSvnSecurityInvocation(args, resolveSvnSecurityContext(cwd));
  ...
  return new Promise((resolve, reject) => {
    if (options.signal?.aborted) {
      resolve({ command: svnPath, args: invocation.safeArgs, cwd, exitCode: null, stdout: '', stderr: '操作已取消。', durationMs: 0, cancelled: true });
      return;
    }
    ...
    child.on('close', (exitCode) => {
      ...
      const stderr = cancelled ? '操作已取消。' : truncated ? `${capturedStderr}${capturedStderr ? '\n' : ''}输出超过安全上限，进程已终止。` : capturedStderr;
