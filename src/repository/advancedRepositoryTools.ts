...
export function parseSvnListXml(xml: string): RepositoryBrowserEntry[] {
  ...
  for (const match of xml.matchAll(expression)) {
    ...
    const commit = /<commit\s+revision="([^"]+)"\s*>([\s\S]*?)<\/commit>/.exec(body);
    entries.push({
  ...
  return entries.filter((entry) => entry.name).sort((left, right) => left.kind === right.kind ? left.name.localeCompare(right.name) : left.kind === 'dir' ? -1 : 1);
}

export function validateRepositoryUrl(value: string, repositoryRoot?: string): string[] {
  const issues: string[] = [];
  ...
  if (normalized && !['http:', 'https:', 'svn:', 'svn+ssh:', 'file:'].includes(normalized.protocol)) {
    issues.push('只允许 http、https、svn、svn+ssh 或 file 仓库 URL。');
  }
  if (normalized && repositoryRoot && !stripTrailingSlash(value).startsWith(`${stripTrailingSlash(repositoryRoot)}/`) && stripTrailingSlash(value) !== stripTrailingSlash(repositoryRoot)) {
    issues.push('目标 URL 必须位于当前 SVN 仓库根地址内。');
  }
  return issues;
}

export function validatePatchText(value: string, maxBytes = 20 * 1024 * 1024): string[] {
  const issues: string[] = [];
  if (!value.trim()) issues.push('补丁文件为空。');
  if (Buffer.byteLength(value, 'utf8') > maxBytes) issues.push(`补丁超过 ${Math.floor(maxBytes / 1024 / 1024)} MB 安全上限。`);
  if (value.includes('\0')) issues.push('补丁包含二进制空字节，工作台不自动应用。');
...
): { markdown: string; count: number; fromRevision?: string; toRevision?: string } {
  const from = parseRevision(fromRevision);
  const to = parseRevision(toRevision);
  const lower = from !== undefined && to !== undefined ? Math.min(from, to) : from;
  const upper = from !== undefined && to !== undefined ? Math.max(from, to) : to;
  const selected = revisions
    .filter((item) => {
      const revision = Number(item.revision);
      return Number.isFinite(revision) && (lower === undefined || revision >= lower) && (upper === undefined || revision <= upper);
    })
    .sort((left, right) => Number(right.revision) - Number(left.revision));
  const lines = ['# SVN 发布说明', '', `修订范围：${lower === undefined ? '全部已加载' : `r${lower}`} → ${upper === undefined ? '最新' : `r${upper}`}`, ''];
  if (selected.length === 0) lines.push('_所选范围没有已加载修订。_');
  for (const revision of selected) {
    const source = repositoryUrl ? `（来源：${stripTrailingSlash(repositoryUrl)}@${revision.revision}）` : '';
    lines.push(`## r${revision.revision} · ${revision.author || 'unknown'} ${source}`, '', revision.message || '（无提交说明）', '', `变更路径：${revision.changedPaths.length}`, ...revision.changedPaths.slice(0, 20).map((item) => `- ${item.action} ${item.path}`), '');
  }
  return {
