...
export function buildEnvironmentDiagnosticReport(input: EnvironmentDiagnosticInput): EnvironmentDiagnosticReport {
  const checks: EnvironmentDiagnosticCheck[] = [
    buildPlatformCheck(input.platform),
    {
      ...
      detail: input.arch,
      action: input.arch === 'x64' || input.arch === 'arm64' ? undefined : '建议在 x64 或 arm64 环境完成正式验收。'
    },
    {
      id: 'vscode',
      label: 'VS Code 版本',
      ...
      action: input.vscodeVersion ? undefined : '需要在 VS Code Extension Host 中运行。'
    },
...
export function formatEnvironmentDiagnosticReport(report: EnvironmentDiagnosticReport): string {
...
export function summarizeEnvironmentStatus(checks: EnvironmentDiagnosticCheck[]): EnvironmentDiagnosticStatus {
...
function buildPlatformCheck(platform: NodeJS.Platform): EnvironmentDiagnosticCheck {
  if (platform === 'win32') {
    return {
      id: 'platform',
      label: '操作系统',
...
function buildSvnCliCheck(
  ...
  return {
    ...
    label: 'SVN CLI',
    status: 'fail',
    detail: configuredSvnPath ? `未找到配置路径：${configuredSvnPath}` : '未找到 svn 可执行文件',
    action: '安装 SVN CLI，或配置 svnWorkbench.svn.path 指向 svn 可执行文件。'
  };
}

function buildWorkspaceCheck(workspaces: EnvironmentDiagnosticWorkspace[]): EnvironmentDiagnosticCheck {
  ...
  const svnWorkspaces = workspaces.filter((workspace) => workspace.isSvnWorkingCopy);
  ...
  return {
    ...
    action: '确认打开的是 SVN 工作副本根目录，或在资源管理器中选择 SVN 工作副本内的文件夹。'
  };
}

function buildAiCheck(ai: EnvironmentDiagnosticAiConfig | undefined): EnvironmentDiagnosticCheck {
  if (!ai) {
    return {
      id: 'ai-config',
      label: 'AI 配置',
  ...
  return {
    ...
    action: '需要 AI 筛选、AI 拆分提交或 AI 冲突建议时，执行 SVN：AI 配置模型。'
  };
}
