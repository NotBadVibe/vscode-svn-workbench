    ...
    items: [
      {
        ...
        title: "命令面板环境检查",
        description:
          "确认扩展在当前 VS Code 用户环境中可以读取 SVN、工作区和 AI 配置状态。",
        steps: [
          "打开命令面板。",
          "执行 SVN：检查环境。",
          "打开 SVN 工作台输出面板。",
      ...
      },
      {
        ...
        expected: ["扩展列表包含 local.svn-workbench@0.0.1。"],
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        title: "右键直达具体任务",
        description:
          "验证 Explorer 和编辑器右键按常用、AI、恢复、更多 SVN 操作、设置与诊断分组，并直接打开当前任务。",
        steps: [
      ...
      },
      {
        ...
        title: "右键文件夹提交当前范围",
        description:
          "验证用户右键某个文件夹时，提交页只处理当前文件夹范围内的内容。",
        steps: [
          "打开 SVN 工作副本。",
          "在资源管理器中右键一个子文件夹。",
          "执行 SVN：提交当前范围。",
      ...
      },
      {
        ...
        title: "右键 AI 操作入口",
        description:
          "验证 AI 审查、影响分析、智能拆分和受控代理可以从资源管理器当前范围触发。",
        steps: [
    ...
    items: [
      {
        ...
        title: "中文术语、时间与数量",
        description:
          "确认主要信息层级使用一致的简体中文，必要技术名词、路径和命令保留原文。",
        steps: [
      ...
      },
      {
        ...
        title: "中文输入法与特殊路径",
        description:
          "验证中文输入候选不会误触发执行，中文及带空格符号的路径可以完整处理。",
        steps: [
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        title: "三主题与无障碍表达",
        description:
          "抽查 Light、Dark、High Contrast 下的边界、焦点、状态与危险提示。",
        steps: [
          "依次切换浅色、深色和高对比度主题。",
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        steps: ["选择一组候选文件。", "填写提交说明。", "执行预览提交计划。"],
        expected: [
    ...
    items: [
      {
        ...
        title: "冲突列表与 AI 建议",
        description:
          "验证冲突中心能收集 SVN 冲突，并给出受边界约束的 AI 建议。",
        steps: [
          "打开存在冲突的工作副本。",
          "执行 SVN：打开冲突中心。",
          "对冲突文件请求 AI 建议。",
      ...
      },
      {
        ...
        steps: ["在冲突中心选择使用工作副本解决。", "刷新冲突列表。"],
        expected: ["SVN resolve 成功后冲突列表更新。", "SCM 状态同步刷新。"],
      },
    ],
  },
  {
...
export function summarizeAcceptanceChecklist(
  ...
  return {
    ...
    steps: sections.reduce(
      (sum, section) =>
        sum +
        section.items.reduce((inner, item) => inner + item.steps.length, 0),
      0,
    ),
    expectedResults: sections.reduce(
      (sum, section) =>
        sum +
        section.items.reduce((inner, item) => inner + item.expected.length, 0),
      0,
    ),
  };
}
