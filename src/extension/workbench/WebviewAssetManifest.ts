...
export async function readWebviewAssets(
  ...
  const localResourceRoot = vscode.Uri.joinPath(context.extensionUri, 'dist', 'webview');
  const manifestUri = vscode.Uri.joinPath(localResourceRoot, '.vite', 'manifest.json');
  let manifest: ViteManifest;

  try {
    const contents = await vscode.workspace.fs.readFile(manifestUri);
    manifest = JSON.parse(Buffer.from(contents).toString('utf8')) as ViteManifest;
  } catch (error) {
    throw new Error(
  ...
  return {
    scriptUri: webview.asWebviewUri(vscode.Uri.joinPath(localResourceRoot, entry.file)),
    styleUris: (entry.css ?? []).map((file) => webview.asWebviewUri(vscode.Uri.joinPath(localResourceRoot, file))),
    localResourceRoot
  };
}
