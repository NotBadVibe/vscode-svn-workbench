...
export async function collectConflictItems(svnPath: string, scope: OperationScope): Promise<SvnConflictItem[]> {
  const byPath = new Map<string, SvnConflictItem>();

  for (const root of scope.roots) {
    const statusResult = await runSvnCommand(svnPath, ['status', '--xml', root.absolutePath], scope.repositoryRoot);
    if (statusResult.exitCode !== 0) {
      throw new Error(statusResult.stderr || `无法采集 ${root.absolutePath} 的 SVN 冲突。`);
    }

    const statusItems = parseStatusXml(statusResult.stdout, scope.repositoryRoot);
    for (const item of statusItems.filter((statusItem) => statusItem.status === 'conflicted')) {
      if (!isPathInScope(scope, item.absolutePath)) {
        continue;
      }

      const infoResult = await runSvnCommand(svnPath, ['info', '--xml', item.absolutePath], scope.repositoryRoot);
      if (infoResult.exitCode !== 0) {
        throw new Error(infoResult.stderr || `无法读取 ${item.absolutePath} 的 SVN 冲突信息。`);
      }

      const conflict = parseConflictInfoXml(infoResult.stdout, item.absolutePath, scope.repositoryRoot);
...
export function parseConflictInfoXml(xml: string, workingFile: string, repositoryRoot: string): SvnConflictItem {
  const conflictTag = /<conflict\s+([^>]+)>/.exec(xml);
  const operation = conflictTag ? getAttribute(conflictTag[1], 'operation') : undefined;
  const type = conflictTag ? getAttribute(conflictTag[1], 'type') : undefined;

  return {
    absolutePath: path.resolve(workingFile),
    relativePath: normalizeRelativePath(path.relative(repositoryRoot, workingFile) || path.basename(workingFile)),
    operation,
    type,
