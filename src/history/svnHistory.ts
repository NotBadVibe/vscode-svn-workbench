...
export async function collectSvnHistory(
  ...
  for (const root of scope.roots) {
    ...
    if (result.exitCode !== 0) {
      throw new Error(result.stderr || `无法读取 ${root.relativePath} 的 SVN 历史。`);
    }
    for (const revision of parseSvnLogXml(result.stdout)) {
      ...
      const keys = new Set(existing.changedPaths.map((item) => `${item.action}:${item.path}`));
  ...
  return [...byRevision.values()].sort((left, right) => Number(right.revision) - Number(left.revision));
}
