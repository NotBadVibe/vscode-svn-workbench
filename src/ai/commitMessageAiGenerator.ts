...
export function buildCommitMessageAiRequest(
  ...
  const selected = new Set(selectedPaths.map((filePath) => normalizePathKey(filePath)));
  const diffByPath = new Map(diffSummaries.map((summary) => [normalizePathKey(summary.absolutePath), summary]));
  const files = candidates
    .filter((candidate) => selected.has(normalizePathKey(candidate.absolutePath)))
    .map((candidate) => toCommitMessageFileContext(candidate, diffByPath.get(normalizePathKey(candidate.absolutePath))))
...
export function createMockCommitMessageResult(request: AiCommitMessageRequest): AiCommitMessageResult {
  ...
  if (request.mode === 'completeTemplate' && request.currentMessage?.trim()) {
    const message = completeCommitMessageTemplate(request.currentMessage, request);
    return {
      message,
      summary: '已基于当前模板补全空字段，并保留用户已填写内容。',
      warnings: request.omittedFileCount > 0 ? ['文件较多，AI 请求只包含前 80 个文件。'] : []
    };
  }
  ...
  const statusText = Object.entries(statuses)
    ...
    .join(', ');
  const totalAdded = request.files.reduce((sum, file) => sum + (file.diff?.addedLines ?? 0), 0);
  const totalDeleted = request.files.reduce((sum, file) => sum + (file.diff?.deletedLines ?? 0), 0);
  const samplePaths = request.files.slice(0, 5).map((file) => {
    const diffText = file.diff ? ` (+${file.diff.addedLines}/-${file.diff.deletedLines})` : '';
    return `- ${file.path}${diffText}`;
  }).join('\n');
  const omittedText = request.omittedFileCount > 0 ? `\n- 另有 ${request.omittedFileCount} 个文件未展示给 AI` : '';
  ...
  return {
    ...
    warnings: [
      ...(request.omittedFileCount > 0 ? ['文件较多，AI 请求只包含前 80 个文件。'] : []),
      ...conventionWarnings
    ]
  };
}

export function mergeCommitMessagePreservingUserContent(currentMessage: string, generatedMessage: string): string {
  ...
  return normalizeLines(currentMessage).map((line) => {
    const parsed = parseFieldLine(line);
    if (!parsed || parsed.value.trim()) {
      return line;
    }

    const generatedValue = generatedByField.get(parsed.key) ?? createFieldFallback(parsed.label, currentMessage);
    return generatedValue ? `${parsed.prefix}${generatedValue}` : line;
  }).join('\n').trim();
}

export function normalizeCommitMessageResult(value: Partial<AiCommitMessageResult>): AiCommitMessageResult {
  const message = typeof value.message === 'string' ? value.message.trim() : '';
  const summary = typeof value.summary === 'string' ? value.summary.replace(/\s+/g, ' ').trim() : '';
  const warnings = Array.isArray(value.warnings)
    ? value.warnings.filter((item): item is string => typeof item === 'string').map((item) => item.trim()).filter(Boolean)
    : [];
...
function completeCommitMessageTemplate(currentMessage: string, request: AiCommitMessageRequest): string {
...
function createGeneratedTemplateMessage(request: AiCommitMessageRequest): string {
...
function createFallbackTitle(request: AiCommitMessageRequest): string {
  ...
  const prefix = convention.requiredPrefix ? convention.allowedPrefixes[0] : undefined;
  const moduleName = convention.requiredModule ? convention.allowedModules[0] : undefined;
...
function getConventionWarnings(request: AiCommitMessageRequest): string[] {
  ...
  if (convention.requiredIssueId) {
    warnings.push(`团队规范要求提交说明包含真实工单号，格式需匹配：${convention.issueIdPattern}。`);
  }
  if (convention.hint) {
...
function inferTitleFromMessage(currentMessage: string): string {
  ...
  return firstMeaningfulLine ? parseFieldLine(firstMeaningfulLine)?.value.trim() ?? '整理当前变更' : '整理当前变更';
}

function getDiffTotals(request: AiCommitMessageRequest): { added: number; deleted: number } {
...
function parseFieldLine(line: string): { key: string; label: string; prefix: string; value: string } | undefined {
