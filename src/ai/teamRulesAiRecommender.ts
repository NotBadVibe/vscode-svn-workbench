...
export function createLocalTeamRulesRecommendation(
  request: AiTeamRulesRequest,
): AiTeamRulesRecommendation {
  const modules = inferModules(request);
  const config: CommitConventionConfig = mergeCommitConventionConfig(
    defaultCommitConventionConfig,
    {
      enabled: true,
      requiredPrefix: true,
      ...
      issueIdPattern:
        request.currentConvention?.issueIdPattern ||
        defaultCommitConventionConfig.issueIdPattern,
    },
  );
  const convention =
    toAiCommitConventionHint(config) ??
    toAiCommitConventionHint({
      ...config,
      enabled: true,
    });
  ...
  return {
    ...
    confidence:
      modules.length >= 3 ? "high" : modules.length > 0 ? "medium" : "low",
  };
}

export function normalizeTeamRulesRecommendation(
  value: Partial<AiTeamRulesRecommendation>,
): AiTeamRulesRecommendation {
  ...
  const safeConfig = validation.valid
    ? config
    : aiConventionToConfig(fallback.commitConvention);
  const convention =
    toAiCommitConventionHint(safeConfig) ?? fallback.commitConvention;
  convention.hint =
    rawConvention?.hint?.trim() || buildCommitConventionHint(safeConfig);
...
function aiConventionToConfig(
  value: AiCommitConventionHint,
): CommitConventionConfig {
  return {
...
async function scanRepository(
  repositoryRoot: string,
): Promise<{ directories: string[]; sampleFiles: string[] }> {
  ...
  async function visit(current: string, depth: number): Promise<void> {
    if (
      ...
      sampleFiles.length >= MAX_SAMPLE_FILES
    ) {
      return;
    }
...
function hasPathSignal(request: AiTeamRulesRequest, pattern: RegExp): boolean {
  return [...request.directories, ...request.sampleFiles].some((item) =>
    pattern.test(item),
  );
}
