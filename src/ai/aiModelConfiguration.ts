...
export const AI_PROVIDER_PRESETS: AiProviderPreset[] = [
  ...
  },
  {
    ...
    model: 'qwen-plus',
    description: '阿里云百炼的 OpenAI 兼容接口；部分账号需要使用对应地域的接口地址。'
  },
  {
    id: 'zhipuCoding',
...
export const AI_USAGE_SCENARIOS: Array<{
  ...
  },
  {
    id: 'conflictAdvice',
    label: '冲突处理建议',
    description: '用于分析基础版本、我的版本、对方版本和工作副本内容，并给出决策建议。'
  },
  {
    id: 'commitMessage',
    label: '提交说明生成',
...
export function validateAiProviderConfig(config: Partial<AiProviderConfig>): AiConfigurationValidation {
  const issues: string[] = [];
...
export async function readStoredAiConfiguration(context: vscode.ExtensionContext): Promise<StoredAiConfiguration> {
  const configuration = vscode.workspace.getConfiguration('svnWorkbench.ai');
  const providerPreset = getAiProviderPreset(configuration.get<string>('providerPreset')).id;
  ...
  const scenarioModels = readScenarioModels(configuration.get<Record<string, string>>('scenarioModels'));
  const secretApiKey = await context.secrets.get(AI_API_KEY_SECRET_KEY);
  const legacyApiKey = configuration.get<string>('apiKey') ?? '';
  const includeCommitHistory = configuration.get<boolean>('includeCommitHistory', false);
  const historyLimit = Math.min(20, Math.max(1, configuration.get<number>('historyLimit', 10)));
...
export async function saveAiConfiguration(
  ...
  const configuration = vscode.workspace.getConfiguration('svnWorkbench.ai');
  await configuration.update('providerPreset', input.providerPreset, vscode.ConfigurationTarget.Global);
  await configuration.update('baseUrl', normalizeAiBaseUrl(input.baseUrl), vscode.ConfigurationTarget.Global);
  await configuration.update('model', input.model.trim(), vscode.ConfigurationTarget.Global);
  await configuration.update('scenarioModels', cleanScenarioModels(input.scenarioModels), vscode.ConfigurationTarget.Global);
  await configuration.update('includeCommitHistory', input.includeCommitHistory === true, vscode.ConfigurationTarget.Global);
  await configuration.update('historyLimit', Math.min(20, Math.max(1, Math.round(input.historyLimit ?? 10))), vscode.ConfigurationTarget.Global);
...
export async function resolveAiProviderConfig(
  ...
  if (!validation.valid) {
    throw new Error(`AI 提供方尚未配置。请执行“SVN：AI 配置模型”。${validation.issues.join(' ')}`);
  }
  return providerConfig;
}

function readScenarioModels(value: Record<string, string> | undefined): Partial<Record<AiUsageScenario, string>> {
  return cleanScenarioModels(value);
}

function cleanScenarioModels(
  value: Partial<Record<AiUsageScenario, string>> | Record<string, string> | undefined
