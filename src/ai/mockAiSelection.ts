...
export function createMockAiSelection(scope: OperationScope, candidates: CommitCandidate[]): AiSelectionResult {
...
export function countAiSelection(result: AiSelectionResult): number {
  return result.recommended.length + result.excluded.length + result.needsReview.length + result.blocked.length;
}
