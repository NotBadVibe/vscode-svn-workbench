...
export function createMockConflictAdvice(
  ...
  if (mine && theirs && normalizeText(mine) === normalizeText(theirs)) {
    return {
      ...
      confidence: "high",
      summary:
        "我的版本与对方版本内容一致，工作副本内容可作为解决结果继续确认。",
...
export function normalizeConflictAdvice(
  value: Partial<AiConflictAdvice>,
): AiConflictAdvice {
...
export function containsSvnConflictMarkers(content: string): boolean {
  return (
    /^<{7}/m.test(content) && /^={7}$/m.test(content) && /^>{7}/m.test(content)
  );
}
