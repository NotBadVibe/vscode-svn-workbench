...
export function createLocalCommitSplitResult(request: AiCommitSplitRequest): AiCommitSplitResult {
  ...
  const splits = groups.map((files, index) => createLocalSplitSuggestion(request, files, index));
  return {
    splits,
    warnings: [
      ...(request.omittedFileCount > 0 ? [`文件较多，拆分建议只分析前 ${MAX_FILES_IN_COMMIT_SPLIT_REQUEST} 个文件。`] : []),
      ...(splits.length <= 1 ? ['当前文件范围较集中，暂不建议强行拆分。'] : [])
    ]
  };
}

export function normalizeCommitSplitResult(value: Partial<AiCommitSplitResult>): AiCommitSplitResult {
  return {
    splits: Array.isArray(value.splits)
      ? value.splits.map(normalizeSplitSuggestion).filter((item) => item.paths.length > 0)
      : [],
...
export function validateCommitSplitResult(
  ...
  const splits = result.splits.map((split, index) => {
...
function toCommitSplitFileContext(candidate: CommitCandidate): AiCommitSplitFileContext {
  return {
...
function groupFilesForSplit(files: AiCommitSplitFileContext[]): AiCommitSplitFileContext[][] {
...
function createLocalSplitSuggestion(
  ...
  return {
    ...
    message: createSplitCommitMessage(request.convention, label, templateGroup, files),
...
function createSplitCommitMessage(
  ...
  const samplePaths = files.slice(0, 6).map((file) => `- ${file.path}`).join('\n');
...
function inferCommitPrefix(convention: AiCommitConventionHint | undefined, templateGroup: string): string | undefined {
  ...
  return convention.allowedPrefixes.includes(value) ? value : convention.allowedPrefixes[0];
}

function inferCommitModule(convention: AiCommitConventionHint | undefined, label: string): string | undefined {
  if (!convention?.enabled || !convention.requiredModule) {
    return undefined;
  }

  const lastPart = label.split('/').filter(Boolean).pop();
  return lastPart && convention.allowedModules.includes(lastPart) ? lastPart : convention.allowedModules[0];
}

function formatCommitTitle(prefix: string | undefined, moduleName: string | undefined, title: string): string {
...
function inferSplitRisks(files: AiCommitSplitFileContext[]): string[] {
  ...
  if (files.some((file) => file.status === 'unversioned' || file.status === 'unknown')) {
    risks.push('包含未版本控制文件，提交前需确认是否需要 svn add。');
  }
  if (files.some((file) => ['dll', 'exe', 'zip', 'jar'].includes(file.fileType))) {
...
function normalizeSplitSuggestion(value: unknown): AiCommitSplitSuggestion {
  ...
  return {
    ...
    reason: typeof raw.reason === 'string' ? raw.reason.trim() : 'AI 建议拆分。',
...
function toAbsolutePath(scope: OperationScope, filePath: string): string {
  return path.isAbsolute(filePath) ? path.resolve(filePath) : path.resolve(scope.repositoryRoot, filePath);
}
