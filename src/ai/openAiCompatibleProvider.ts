...
export class OpenAiCompatibleProvider implements AiProvider {
  ...
  async listModels(): Promise<AiModelInfo[]> {
    ...
    if (!response.ok) {
      throw new Error(`读取 AI 模型列表失败：${response.status} ${response.statusText}`);
    }
  ...
  async generateCommitMessage(request: AiCommitMessageRequest): Promise<AiCommitMessageResult> {
    const content = [
    ...
    const text = await this.complete(content);
    return normalizeCommitMessageResult(parseJsonObject(text) as Partial<AiCommitMessageResult>);
  }

  async suggestCommitSplits(request: AiCommitSplitRequest): Promise<AiCommitSplitResult> {
    const content = [
    ...
    const text = await this.complete(content);
    return normalizeCommitSplitResult(parseJsonObject(text) as Partial<AiCommitSplitResult>);
  }

  async recommendTeamRules(request: AiTeamRulesRequest): Promise<AiTeamRulesRecommendation> {
    const content = [
    ...
    const text = await this.complete(content);
    return normalizeTeamRulesRecommendation(parseJsonObject(text) as Partial<AiTeamRulesRecommendation>);
  }
  ...
  async adviseConflict(request: AiConflictRequest): Promise<AiConflictAdvice> {
    ...
    const text = await this.complete(content);
    return normalizeConflictAdvice(parseJsonObject(text) as Partial<AiConflictAdvice>);
  }
...
function parseJsonResult(text: string): AiSelectionResult {
  return normalizeAiSelectionResult(parseJsonObject(text) as Partial<AiSelectionResult>);
}
