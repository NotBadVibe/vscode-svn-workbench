...
export function readTeamMemory(storage: Memento, repositoryIdentity: string): TeamMemorySnapshot {
  const value = storage.get<unknown>(teamMemoryStorageKey(repositoryIdentity));
  const entries = Array.isArray(value) ? value.map(normalizeEntry).filter((entry): entry is TeamMemoryEntry => Boolean(entry)) : [];
  return { source: '当前仓库成功提交', entries: entries.slice(0, TEAM_MEMORY_MAX_ENTRIES), maxEntries: TEAM_MEMORY_MAX_ENTRIES, externallyShared: false };
}
...
export async function appendTeamMemory(
  ...
  }, ...current.entries.filter((entry) => entry.revision !== input.revision || entry.summary !== summary)].slice(0, TEAM_MEMORY_MAX_ENTRIES);
  await storage.update(teamMemoryStorageKey(repositoryIdentity), next);
  return { ...current, entries: next };
}

export async function clearTeamMemory(storage: Memento, repositoryIdentity: string): Promise<void> {
...
function normalizeEntry(value: unknown): TeamMemoryEntry | undefined {
  ...
  const summary = typeof raw.summary === 'string' ? sanitizeSummary(raw.summary) : '';
  const recordedAt = typeof raw.recordedAt === 'string' && !Number.isNaN(Date.parse(raw.recordedAt)) ? raw.recordedAt : undefined;
  if (!summary || !recordedAt || raw.source !== 'successful-commit') return undefined;
  return { revision: normalizeRevision(raw.revision), summary, recordedAt, source: 'successful-commit' };
}
