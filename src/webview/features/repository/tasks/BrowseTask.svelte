...
<section class="operation-card operation-card--wide repository-task-card">
  ...
  <div class="repository-browser-toolbar">
    ...
    ><button
      class="button button--secondary"
      onclick={() => openBrowser(snapshot.info.url ?? "")}>返回当前路径</button
    >
  </div>
  {#if snapshot.advanced.browser}
    <div class="browser-location">
      <button
        ...
        ></span></button
      ><code>{snapshot.advanced.browser.url}</code><button
        class="icon-button"
    ...
    <ScrollArea class="repository-browser-list" label="仓库目录内容"
      >{#if snapshot.advanced.browser.entries.length === 0 && !snapshot.advanced.browser.error}<div
          class="mini-empty"
        >
          这个仓库目录为空。
        </div>{/if}{#each snapshot.advanced.browser.entries as entry (entry.name)}<button
          onclick={() =>
            entry.kind === "dir"
              ? openBrowser(childUrl(entry.name))
          ...
          ></span><strong>{entry.name}</strong><small
            >r{entry.revision ?? "?"} · {entry.author ?? "未知"}</small
          ><span>{entry.kind === "dir" ? "打开" : "复制 URL"}</span></button
        >{/each}</ScrollArea
    >
