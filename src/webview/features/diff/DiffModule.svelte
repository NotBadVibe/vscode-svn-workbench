<script lang="ts">
  ...
  import type { DiffSnapshot, WebviewAction } from '@protocol/workbenchProtocol';
  ...
  $effect(() => {
    ...
    const mergeView = new MergeView({
      a: {
        doc: original,
        extensions: [...readonlyExtensions, EditorView.contentAttributes.of({ 'aria-label': `${snapshot.relativePath} BASE 内容` })]
      },
      b: {
        doc: modified,
        extensions: [...readonlyExtensions, EditorView.contentAttributes.of({ 'aria-label': `${snapshot.relativePath} 工作副本内容` })]
      },
      parent,
      gutter: true,
...
<section class="feature-layout diff-feature">
  <div class="feature-toolbar">
    ...
    <div class="toolbar-actions">
      <button class="button button--secondary" onclick={() => onAction('open-file', { relativePath: snapshot.relativePath })}>
        在编辑器中打开
      </button>
      <button class="button button--primary" onclick={() => onAction('open-module', { moduleId: 'commit', selectedPaths: [snapshot.relativePath] })}>
        提交此文件
      </button>
    </div>
  </div>
  ...
  {#if snapshot.message}
    <div class="notice" class:notice--warning={snapshot.truncated || snapshot.binary} role="status">
  ...
  {:else if snapshot.language === 'diff'}
    <pre class="unified-diff" aria-label={`${snapshot.relativePath} 统一差异`}><code>{snapshot.modified}</code></pre>
  {:else}
    <div class="merge-view-frame" aria-label={`${snapshot.relativePath} 差异`}>
      <div class="merge-view-labels" aria-hidden="true"><span>BASE</span><span>工作副本</span></div>
