...
export class SvnSourceControlManager implements vscode.Disposable {
  ...
  private async discoverRepositories(): Promise<void> {
    ...
    for (const folder of vscode.workspace.workspaceFolders ?? []) {
      ...
      for (const item of metadata)
        roots.add(path.dirname(path.dirname(item.fsPath)));
    }
    ...
    for (const root of roots) {
      if (!this.repositories.has(root))
        this.repositories.set(root, this.createRepository(root));
    }
  ...
  private createRepository(root: string): RepositoryModel {
    ...
    const unversioned = sourceControl.createResourceGroup(
      "unversioned",
      "未版本化",
    );
  ...
  private async refreshRepository(repository: RepositoryModel): Promise<void> {
    try {
      const scope: OperationScope = {
        ...
        source: "scmSelection",
        roots: [
          { absolutePath: repository.root, relativePath: ".", kind: "folder" },
        ],
      ...
      repository.conflicts.resourceStates = states.filter(
        (item) => item.contextValue === "svnWorkbench.conflicted",
      );
      repository.unversioned.resourceStates = states.filter(
        (item) => item.contextValue === "svnWorkbench.unversioned",
      );
      repository.changes.resourceStates = states.filter(
        (item) =>
      ...
      appendOutput(
        `刷新 ${repository.root} 的 SCM 状态失败：${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }

  private toResourceState(
    candidate: Awaited<ReturnType<typeof collectCommitCandidates>>[number],
  ...
  private scheduleRefresh(uri: vscode.Uri | undefined): void {
    ...
    if (
      ![...this.repositories.values()].some((item) =>
        isInside(item.root, uri.fsPath),
      )
    )
      return;
    if (this.refreshTimer) clearTimeout(this.refreshTimer);
