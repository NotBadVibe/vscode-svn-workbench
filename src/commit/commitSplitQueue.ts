...
export type CommitSplitQueueItemStatus =
  "pending" | "applied" | "submitting" | "completed" | "failed";
...
export type CommitSplitQueueApplyBlockReason =
  "notPreviewed" | "blocked" | "submitting" | "completed";
export type CommitSplitQueueRetryBlockReason =
  CommitSplitQueueApplyBlockReason | "notFailed";
...
export type CommitSplitQueuePreviewIssueCategory =
  ...
  | "emptySelection"
  | "unknown";
export type CommitSplitQueuePreviewIssueCategoryFilter =
  "all" | CommitSplitQueuePreviewIssueCategory;
...
export type CommitSplitQueueSubmitGuard = CommitSplitQueueApplyGuard;
...
export function addCommitSplitToQueue(
  ...
  const existing = queue.find(
    (item) => getSuggestionQueueKey(item.suggestion) === key,
  );
  if (existing) {
    return queue;
  }
...
export function addCommitSplitsToQueue(
  ...
  const existingKeys = new Set(
    queue.map((item) => getSuggestionQueueKey(item.suggestion)),
  );
...
export function removeCommitSplitFromQueue(
  queue: CommitSplitQueueItem[],
  id: string,
): CommitSplitQueueItem[] {
...
export function removeCompletedCommitSplitQueueItems(
  queue: CommitSplitQueueItem[],
): CommitSplitQueueItem[] {
  return queue.filter((item) => item.status !== "completed");
}

export function markCommitSplitQueueItemApplied(
  queue: CommitSplitQueueItem[],
  id: string,
): CommitSplitQueueItem[] {
...
export function canApplyCommitSplitQueueItem(
  item: CommitSplitQueueItem,
): CommitSplitQueueApplyGuard {
...
export function canSubmitCommitSplitQueueItem(
  item: CommitSplitQueueItem,
): CommitSplitQueueSubmitGuard {
...
export function canRetryCommitSplitQueueItem(
  item: CommitSplitQueueItem,
): CommitSplitQueueRetryGuard {
...
export function markCommitSplitQueueItemSubmitting(
  queue: CommitSplitQueueItem[],
  id: string,
): CommitSplitQueueItem[] {
  return queue.map((item) =>
...
export function refreshCommitSplitQueueAfterCommit(
  queue: CommitSplitQueueItem[],
): CommitSplitQueueItem[] {
  return queue.map((item) => {
    if (item.status === "completed") {
  ...
  return queue
    ...
    .filter((item) =>
      doesCommitSplitQueueItemMatchPreviewIssueCategory(
        item,
        previewIssueCategoryFilter,
      ),
    );
}

export function getRepreviewableCommitSplitQueueItems(
  queue: CommitSplitQueueItem[],
): CommitSplitQueueItem[] {
  return queue.filter(
    (item) =>
...
export function getNotPreviewedCommitSplitQueueItems(
  queue: CommitSplitQueueItem[],
): CommitSplitQueueItem[] {
  return getRepreviewableCommitSplitQueueItems(queue).filter(
    (item) => item.planStatus === "notPreviewed",
  );
}

export function getFailedRepreviewableCommitSplitQueueItems(
  queue: CommitSplitQueueItem[],
): CommitSplitQueueItem[] {
  return getRepreviewableCommitSplitQueueItems(queue).filter(
...
export function createCommitSplitQueueBulkPreviewState(
  ...
  const ids = getRepreviewableCommitSplitQueueItems(queue).map(
    (item) => item.id,
  );
  if (ids.length === 0) {
    return undefined;
...
export function summarizeCommitSplitQueueBulkPreview(
  ...
  const completed = state.completedIds.filter((id) =>
...
export function summarizeCommitSplitQueueBulkPreviewResult(
  ...
  return {
    ...
    notPreviewed: items.filter((item) => item.planStatus === "notPreviewed")
      .length,
...
export function collectCommitSplitQueuePreviewIssues(
  queue: CommitSplitQueueItem[],
...
export function doesCommitSplitQueueItemMatchPreviewIssueCategory(
  ...
  const issues =
    item.lastPreviewIssues && item.lastPreviewIssues.length
      ? item.lastPreviewIssues
      : [{ reason: "该拆分项预览未通过，请重新预览查看详情。" }];
  return issues.some(
    (issue) => classifyCommitSplitQueuePreviewIssue(issue.reason) === category,
  );
}
    ...
    const issuePaths = issues
      .filter(
        (issue) =>
          issue.path &&
          (category === "all" ||
            classifyCommitSplitQueuePreviewIssue(issue.reason) === category),
      )
...
export function classifyCommitSplitQueuePreviewIssue(
  reason: string,
): CommitSplitQueuePreviewIssueCategory {
...
export function getCommitSplitQueuePreviewIssueCategoryLabel(
  category: CommitSplitQueuePreviewIssueCategory,
): string {
  switch (category) {
    case "scope":
  ...
  switch (category) {
    case "scope":
      return {
        ...
        title: "处理范围不匹配",
        detail:
          "拆分项包含当前右键范围外的文件。建议切到更高层级提交，或让 AI 重新生成只包含当前范围的拆分建议。",
    ...
    case "candidate":
      return {
        ...
        title: "处理候选列表缺失",
        detail:
          "拆分项包含不在当前 SVN 候选列表中的文件。建议先刷新状态；如果文件已移动、删除或不属于本次范围，重新生成拆分建议。",
    ...
    case "excluded":
      return {
        ...
        title: "处理规则排除",
        detail:
          "文件被生成文件规则或团队预设排除。建议确认是否确实需要提交；需要提交时先调整规则或白名单，再重新预览。",
    ...
    case "blocked":
      return {
        ...
        title: "处理阻止状态",
        detail:
          "文件处于冲突、缺失、阻塞或异常状态。建议先在冲突中心或 SVN 状态视图处理异常，再重新预览拆分计划。",
    ...
    case "svnStatus":
      return {
        ...
        title: "处理 SVN 状态不支持",
        detail:
          "当前 SVN 状态不能直接提交。建议根据状态执行 add、remove、revert、resolved 或 update，让文件回到可提交状态。",
    ...
    case "emptySelection":
      return {
        ...
        title: "处理空选择",
        detail:
          "拆分项没有可进入提交计划的文件。建议重新选择文件，或让 AI 基于当前候选列表重新生成拆分建议。",
    ...
    default:
      return {
        ...
        title: "处理其他原因",
        detail:
          "当前阻止项缺少明确分类。建议先重新预览获取最新详情；如果仍然无法识别，再按具体文件状态人工判断。",
...
function createRefreshAndRepreviewAction(
  label: string,
...
function createManualReviewAction(
  label: string,
): CommitSplitQueuePreviewIssueQuickAction {
  return {
    kind: "manualReview",
...
export function summarizeCommitSplitQueue(
  ...
  const visible = getVisibleCommitSplitQueueItems(
    queue,
    hideCompleted,
    filter,
    planFilter,
  ...
  return {
    ...
    notPreviewed: queue.filter((item) => item.planStatus === "notPreviewed")
      .length,
...
export function getCommitSplitQueueNextAction(
  queue: CommitSplitQueueItem[],
  bulkPreviewSummary: CommitSplitQueueBulkPreviewSummary = summarizeCommitSplitQueueBulkPreview(
    undefined,
  ),
  ...
  const actionable = queue.filter(
    (item) => item.status !== "completed" && item.status !== "submitting",
  );
  const blockedCount = actionable.filter(
    (item) => item.planStatus === "blocked",
  ).length;
  if (blockedCount > 0) {
    return {
      kind: "reviewBlocked",
  ...
  const failedRepreviewable =
    getFailedRepreviewableCommitSplitQueueItems(queue);
  ...
  const notPreviewedCount = actionable.filter(
    (item) => item.planStatus === "notPreviewed",
  ).length;
...
export function getNextCommitSplitQueueItem(
  ...
  const actionable = (item: CommitSplitQueueItem) =>
    item.status !== "completed" && item.status !== "submitting";
  if (!afterId) {
  ...
  return (
    queue.slice(index + 1).find(actionable) ??
    queue.slice(0, index).find(actionable)
  );
}

export function getFirstSubmittableCommitSplitQueueItem(
  queue: CommitSplitQueueItem[],
): CommitSplitQueueItem | undefined {
  return getNextSubmittableCommitSplitQueueItem(queue);
}

export function getFirstRetryableCommitSplitQueueItem(
  queue: CommitSplitQueueItem[],
): CommitSplitQueueItem | undefined {
...
export function getNextSubmittableCommitSplitQueueItem(
  ...
  const submittable = (item: CommitSplitQueueItem) =>
    canSubmitCommitSplitQueueItem(item).allowed;
  if (!afterId) {
  ...
  return (
    queue.slice(index + 1).find(submittable) ??
    queue.slice(0, index).find(submittable)
  );
}
...
export function updateCommitSplitQueueItemPreviewStatus(
  ...
  return queue.map((item) =>
    item.id === preview.splitId
      ? {
          ...item,
          planStatus: preview.preview.canCommit
            ? ("ready" as const)
            : ("blocked" as const),
...
export function getSuggestionQueueKey(
  suggestion: AiCommitSplitSuggestion,
): string {
  const paths = suggestion.paths
    .map((filePath) => filePath.replace(/\\/g, "/").toLocaleLowerCase())
    .sort();
