...
import { GeneratedFileDecision, classifyGeneratedFile } from './generatedFilePolicy';

export type CommitCandidateSelection = 'selected' | 'needsReview' | 'excluded' | 'blocked';
export type CommitTemplateGroup = 'frontend' | 'backend' | 'document' | 'config' | 'asset' | 'other';
...
export async function collectCommitCandidates(svnPath: string, scope: OperationScope): Promise<CommitCandidate[]> {
  const byPath = new Map<string, CommitCandidate>();

  for (const root of scope.roots) {
    const result = await runSvnCommand(svnPath, ['status', '--xml', root.absolutePath], scope.repositoryRoot);
    if (result.exitCode !== 0) {
      throw new Error(result.stderr || `无法采集 ${root.absolutePath} 的 SVN 状态。`);
    }
...
export function summarizeCommitCandidates(candidates: CommitCandidate[]): CommitCandidateSummary {
...
function inferSelection(status: SvnStatus, generatedDecision: GeneratedFileDecision): CommitCandidateSelection {
  if (status === 'conflicted' || status === 'obstructed' || status === 'incomplete') {
    return 'blocked';
  }

  if (generatedDecision === 'exclude' || status === 'ignored' || status === 'external' || status === 'normal') {
    return 'excluded';
  }

  if (generatedDecision === 'review' || status === 'missing' || status === 'unversioned' || status === 'unknown') {
    return 'needsReview';
  }
...
function inferFileType(absolutePath: string): string {
  ...
  const extension = path.extname(absolutePath).replace('.', '').toLocaleLowerCase();
  return extension || 'no-ext';
}

function inferTemplateGroup(relativePath: string, fileType: string): CommitTemplateGroup {
  const normalized = relativePath.split(path.sep).join('/').toLocaleLowerCase();
  if (normalized.startsWith('docs/') || fileType === 'md' || fileType === 'txt') {
    return 'document';
  }
  if (['vue', 'tsx', 'ts', 'jsx', 'js', 'scss', 'css', 'html'].includes(fileType)) {
    return 'frontend';
  }
  if (['java', 'kt', 'cs', 'go', 'py', 'php', 'rb'].includes(fileType)) {
    return 'backend';
  }
  if (['json', 'yml', 'yaml', 'toml', 'ini', 'xml', 'config'].includes(fileType)) {
    return 'config';
  }
