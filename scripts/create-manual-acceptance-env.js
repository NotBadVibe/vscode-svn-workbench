...
fs.writeFileSync(
  path.join(validationRoot, "README-manual-acceptance.md"),
  readme,
  "utf8",
);
...
function ensureSafeValidationPath(target) {
  ...
  const allowed =
    process.platform === "win32"
      ? /^([A-Z]:\\)?svn-workbench-manual-ui-acceptance-v2$/i.test(resolved) ||
        /\\svn-workbench-manual-ui-acceptance-v2$/i.test(resolved)
      : resolved === "/tmp/svn-workbench-manual-ui-acceptance-v2" ||
        resolved.endsWith("/svn-workbench-manual-ui-acceptance-v2");
  if (!allowed) {
    throw new Error(
