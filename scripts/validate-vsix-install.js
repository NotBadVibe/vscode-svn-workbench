...
if (!installedExtensions.includes(expectedExtension)) {
  throw new Error(
    `Expected ${expectedExtension} in clean profile, got: ${installedExtensions.join(", ")}`,
  );
}
...
if (afterUninstall.includes(expectedExtension)) {
  throw new Error(
    `Expected ${expectedExtensionId} to be removed, got: ${afterUninstall.join(", ")}`,
  );
}
...
if (!afterReinstall.includes(expectedExtension)) {
  throw new Error(
    `Expected ${expectedExtension} after reinstall, got: ${afterReinstall.join(", ")}`,
  );
}
...
fs.writeFileSync(
  path.join(acceptanceRoot, "latest-summary.json"),
  `${JSON.stringify(summary, null, 2)}\n`,
  "utf8",
);
...
function resolveCodeCommand() {
  ...
  return (
    candidates.find((candidate) =>
      candidate.toLocaleLowerCase().endsWith(".cmd"),
    ) ??
    candidates[0] ??
    "code.cmd"
  );
